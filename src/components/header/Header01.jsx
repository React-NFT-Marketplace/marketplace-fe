...
export default function Header01({handleAccountChange, handleChainChange}) {
  ...
  const setToggleBalanceWindow = useCallback(() => {
    setShowBalanceWindow(!showBalanceWindow)
  }, [showBalanceWindow]);
      ...
      <header className="js-page-header fixed top-0 z-20 w-full backdrop-blur transition-colors">
        <div className="flex items-center px-6 py-6 xl:px-24 ">
          ...
          <div className="ml-auto flex lg:hidden">
            <div className={`js-nav-dropdown group-dropdown relative ${account? '' : 'hidden'}`}>
                <button onClick={setToggleBalanceWindow} className="dropdown-toggle border-jacarta-100 hover:bg-accent focus:bg-accent group dark:hover:bg-accent ml-2 flex h-10 w-10 items-center justify-center rounded-full border bg-white transition-colors hover:border-transparent focus:border-transparent dark:border-transparent dark:bg-white/[.15]">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 24 24"
                    width={24}
                ...
                <div className={`dropdown-menu dark:bg-jacarta-800 group-dropdown-hover:opacity-100 group-dropdown-hover:visible !-right-4 !top-[85%] !left-auto z-10 min-w-[14rem] whitespace-nowrap rounded-xl bg-white transition-all will-change-transform before:absolute before:-top-3 before:h-3 before:w-full lg:absolute lg:grid lg:!translate-y-4 lg:py-4 lg:px-2 lg:shadow-2xl ${showBalanceWindow ? '' : 'hidden'} lg:invisible lg:opacity-0`}>
                  <div>
                    <button
                      className="js-copy-clipboard font-display text-jacarta-700 my-4 flex select-none items-center whitespace-nowrap px-5 leading-none dark:text-white"
                      onClick={() => { copyToClipboard(account); }}
